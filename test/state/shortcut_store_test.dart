...
void main() {
  ...
  group('entriesSorted', () {
    ...
    test(
      'favoritesFirst puts favorited entries before non-favorited entries',
      () async {
        final List<ShortcutEntry> seed = <ShortcutEntry>[
          _seedEntry(id: 'a', name: 'Alpha'),
  ...
  test(
    'adds and updates shortcut with custom tags and favorite flag',
    () async {
      final ShortcutStore store = buildStore();
  ...
  test(
    'persists curated theme preferences (amoled, warmSepia, forestDark, cyberpunkNeon)',
    () async {
      final MemoryShortcutRepository repository = MemoryShortcutRepository();
      final ShortcutStore store = buildStore(repository);
