# Features — SreerajP YouTube Shortcuts (YT Shortcuts)

This document provides a comprehensive, authoritative breakdown of all features, system integrations, data models, link processing capabilities, UI workflows, and accessibility assurances available in **YT Shortcuts**.
...
- **Note on app name**: the app shows three different name strings depending on where you look —
  ...
  in-app `MaterialApp` title and this About screen's header both show "SreerajP YouTube
  Shortcuts", while the Home screen's own app bar title shows "YT Shortcuts". This is expected
